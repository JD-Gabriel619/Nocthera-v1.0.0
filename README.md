# Nocthera - Ultimate Discord Bot

**Nocthera** is a powerful, feature-rich Discord bot designed to enhance your server experience with comprehensive moderation tools, engaging economy systems, utility features, and much more. Built with modern Discord.js v14 and PostgreSQL for optimal performance and data persistence.

[![Support Server](https://img.shields.io/badge/-Support%20Server-%235865F2?logo=discord&logoColor=white&style=flat-square&logoWidth=20)](https://discord.gg/jJX9GWg7v2)
...
- [Support Server](https://discord.gg/jJX9GWg7v2)
...
Nocthera offers a complete suite of tools for Discord server management and community engagement:
...
[**Nocthera Setup Tutorial**](https://www.youtube.com/@TouchDisc)

## Docker Deployment (Recommended)

Nocthera is fully containerized for easy deployment.

1. **Clone the repository:**
   ```bash
   git clone https://github.com/JDGabriel/Nocthera.git
   cd Nocthera
   ```
...
2. **Configure environment variables:**
   ...
   cp .env.example .env
   ```
   Set at minimum `DISCORD_TOKEN`, `CLIENT_ID`, and `GUILD_ID`. Docker Compose also reads `POSTGRES_USER`, `POSTGRES_PASSWORD`, and `POSTGRES_DB` from `.env` (defaults: `nocthera` / `password` / `nocthera`).
...
```bash
docker pull ghcr.io/JDGabriel/nocthera:main
```
...
3. **Configure Environment Variables**
   ...
   POSTGRES_URL=postgresql://postgres:yourpassword@localhost:5432/nocthera
   POSTGRES_HOST=localhost
   POSTGRES_PORT=5432
   POSTGRES_DB=nocthera
   ...
   - If your chosen `PORT` is already used, Nocthera automatically tries the next port(s)
...
4. **Setup PostgreSQL Database** (Optional but recommended)
   ...
   createdb nocthera
   createuser nocthera
   psql -c "ALTER USER nocthera PASSWORD 'yourpassword';"
   psql -c "GRANT ALL PRIVILEGES ON DATABASE nocthera TO nocthera;"
   ```
...
Nocthera requires the following Discord intents:
- **Guilds**
- **Guild Messages**
- **Message Content**
- **Guild Members**
...
Nocthera is released under the MIT License. See [LICENSE](LICENSE) for details.

## Thank You

Thank you for choosing Nocthera for your Discord server! We're constantly working to improve and add new features based on community feedback.
